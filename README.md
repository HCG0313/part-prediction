...
| 현재 결과 | 2026-06-24 단일 검증은 방향 적중 10/12였지만 Top3 겹침은 1/3이다. 누적 실전 성능은 계속 추적 중이다. |
...
오늘은 장중 KIS 연결 실패의 원인을 인증 문제와 endpoint 연결 문제로 분리하고, KIS/Naver fallback 구조를 포트폴리오 관점에서 설명 가능한 형태로 정리했다.  
핵심 결론은 `KIS가 불안정할 때는 Naver로 즉시 대체하되, 데이터 출처를 분리 기록해 예측 실패 원인을 재현 가능하게 남긴다`는 것이다.

| 점검 항목 | 결과 | 증거 |
| --- | --- | --- |
| 2026-06-24 실제 결과 | 평균 섹터 수익률 약 +0.78%, 12개 중 10개 상승. 실제 Top3는 바이오·화학/정유·2차전지 | [2026-06-24 일기](docs/diary/2026-06-24.md) |
| 전일 예측 검증 | 방향 적중 10/12로 양호했지만, 예측 Top3와 실제 Top3의 겹침은 화학/정유 1개 | [예측 정확도 요약](reports/prediction_accuracy_summary.json) |
| KIS 연결 점검 | 토큰 발급과 기본시세 호출은 정상, 장중 `9443` 연결은 타임아웃. 장마감 KIS 스냅샷은 240개 성공 | [문제 해결 기록](docs/problem-solving-log.md) |
| 수집 구조 개선 | KIS와 Naver fallback 데이터를 분리하고, 실제 사용 입력은 active 파일에서 출처와 함께 추적하는 방향으로 정리 | [문제 해결 기록](docs/problem-solving-log.md) |
| 2026-06-25 예측 | 상위 후보는 바이오·조선/방산·2차전지·화학/정유·자동차. 랭킹 품질 게이트 `severe`로 최종 행동은 9개 회피 우선, 3개 관망 | [내일 예측 CSV](reports/tomorrow_sector_prediction.csv) |

## 2026-06-22 업그레이드 요약

오늘은 메인 모델을 새로 갈아엎지 않고, 패닉장 이후 먼저 반등하는 섹터가 메인 예측표 안에서 보이지 않는 문제를 보완했다.  
핵심 결론은 `최종 행동은 보수적으로 유지하되, 급락장 안에서 우선 확인할 반등 후보를 별도 관찰 레이어로 표시`하는 것이다.

| 개선 항목 | 결과 | 증거 |
| --- | --- | --- |
| 패닉 반등 관찰 레이어 | `panic_rebound_watch_score`, `panic_rebound_action_label`, `panic_rebound_candidate_rank`를 내일 예측표에 추가 | [예측 레이어 리포트](reports/prediction_action_layer_report.md) |
| 최종 행동 보수성 유지 | 반등 후보를 표시하되 `final_action`은 자동 완화하지 않음. 2026-06-23 기준 반도체/전자는 `방어 추적`이지만 최종 행동은 `관망` | [내일 예측 CSV](reports/tomorrow_sector_prediction.csv) |
| 누락 컬럼 오류 방어 | 구간 보정 padding 컬럼이 없을 때 리포트 생성이 멈추는 문제를 `numeric_series()` 기반 처리로 수정 | [문제 해결 기록](docs/problem-solving-log.md) |
| 오늘 실제 결과 반영 | 2026-06-22 실제 1위는 반도체/전자 +4.51%. 기존 예측 Top3는 실패했고, 이를 패닉 반등 후보 표시 문제로 기록 | [2026-06-22 일기](docs/diary/2026-06-22.md) |

## 최신 운영 기록

| 기준일 | 시장 요약 | 검증 결과 | 다음 거래일 예측 |
| --- | --- | --- | --- |
| 2026-06-24 | 평균 섹터 수익률 +0.78%, 12개 중 10개 상승. 바이오·화학/정유·2차전지가 실제 Top3 | 6월 23일 예측은 방향 적중 10/12로 좋았지만 Top3 겹침은 화학/정유 1개. 2차전지는 예측 12위였으나 실제 3위 | 2026-06-25 기준 상위 후보는 바이오·조선/방산·2차전지·화학/정유·자동차. 랭킹 품질 게이트 severe로 최종 행동은 대부분 회피 우선 |
| 2026-06-23 | 평균 섹터 수익률 -6.13%, 12개 섹터 모두 하락. 유통/소비·바이오·금융이 상대 방어, 반도체/전자·2차전지·통신은 크게 약세 | 6월 22일 예측은 최종 행동이 대부분 회피라 시장 위험 방어는 유효했지만, 예측 1위 반도체/전자는 실제 최하위. Top3 겹침은 바이오 1개 | 2026-06-24 기준 전 섹터 회피 우선. 바이오·유통/소비·금융은 방어 추적 후보이나 신규 진입 신호가 아니라 장초반 회복 확인 대상 |
| 2026-06-22 | 평균 섹터 수익률 -2.00%, 12개 중 반도체/전자만 +4.51%로 상승. 자동차·금융·조선/방산은 하위권 | 6월 19일 예측은 최종 행동 전 섹터 회피 우선이라 시장 방향 방어는 유효했지만, 예측 Top3 조선/방산·자동차·금융은 모두 실패. 실제 Top3와 겹침 0개 | 2026-06-23 기준 반도체/전자가 예측 1위이자 패닉 반등 `방어 추적` 1위. 화학/정유·바이오는 `방어 관찰`, 전체 행동은 11개 회피 우선·1개 관망 |
...
상세 내용은 [2026-06-24 일기](docs/diary/2026-06-24.md)에 정리했습니다.
...
| 학습 기준 | 2026-06-23 장마감 후 |
| 학습 행 수 | 9,156 |
| 사용 특성 수 | 301 |
| Walk-forward Top1 평균 수익률 | 약 0.42% |
| Walk-forward Top1 초과수익률 | 약 0.20% |
| Walk-forward Top3 평균 수익률 | 약 0.28% |
| Walk-forward Top3 양수 비율 | 약 53.4% |
| 최근 실전 방향 정확도 | 약 43.4% |
| 실전 Top3 겹침률 | 약 29.9% |
...
| 날짜 | 상세 기록 | 핵심 내용 |
| --- | --- | --- |
| 2026-06-24 | [2026-06-24 일기](docs/diary/2026-06-24.md) | 반등장 방향 적중 10/12, Top3 겹침 1/3, KIS 연결 원인 분리, KIS/Naver 수집 분리, 6월 25일 바이오·조선/방산 관망 |
| 2026-06-23 | [2026-06-23 일기](docs/diary/2026-06-23.md) | 전 섹터 급락, 반도체/전자 예측 1위 실패, 챔피언-챌린저 운영 기준 정리, 2026-06-24 전 섹터 회피 우선 |
| 2026-06-22 | [2026-06-22 일기](docs/diary/2026-06-22.md) | 반도체/전자 단독 강세, 기존 예측 Top3 실패, 패닉 반등 관찰 레이어 추가, 6월 23일 반도체/전자 방어 추적 |
...
| 문제 | 해결 방향 |
| --- | --- |
| 패닉장 반등 후보가 메인 예측표에 보이지 않음 | `panic_rebound_*` 관찰 레이어를 추가해 반등 후보를 표시하되 최종 행동은 자동 완화하지 않음 |
...
| KIS와 Naver fallback 출처 추적 어려움 | KIS/Naver 수집 파일을 분리하고 active 입력 파일에 실제 사용 출처를 남김 |
...
| 급락장에서 예측 순위가 크게 흔들림 | 메인 목표는 유지하고, V5를 챔피언으로 두며 서브 모델 성능을 시간에 따라 누적 비교하는 운영 기준으로 전환 |
...
| 서브 모델 표본이 아직 짧음 | 새 하이브리드 모델은 즉시 메인화하지 않고 shadow로 설계한 뒤 1주일 이상 비교 |
