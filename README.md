...
최근 업데이트: 2026-06-20 포트폴리오/모델 감시판 기준
...
오늘은 모델 자체를 무리하게 교체하지 않고, **메인 모델과 shadow 모델을 매일 비교할 수 있는 감시판**과 **예측 후보/최종 행동 분리 구조**를 추가했다.  
핵심 결론은 `메인 교체 금지, shadow 비교판으로 누적 관찰`이다.

| 개선 항목 | 결과 | 증거 |
| --- | --- | --- |
| 메인 vs shadow 통합 비교 | 5개 shadow 계열을 같은 기준으로 비교, 현재 교체 후보 0개 | [메인-Shadow 통합 비교판](reports/model_shadow_scoreboard_report.md) |
| 메인 V5 실패 유형 진단 | 315거래일, 3780개 행 기준으로 섹터별/국면별 실패 원인 분해 | [실패 유형 진단](reports/model_failure_diagnostics_report.md) |
| 예측 후보와 최종 행동 분리 | 상승 후보가 곧바로 진입 후보로 오해되지 않도록 표 구조 분리 | [예측/행동 분리표](reports/prediction_decision_separation_report.md) |
| GitHub 포트폴리오 기록 방식 정리 | 문제-기간-원인-해결-검증 구조와 Issue 템플릿 추가 | [GitHub 기록 운영안](docs/github-portfolio-issue-plan.md) |
...
상세 내용은 [2026-06-19 일기](docs/diary/2026-06-19.md)에 정리했습니다.
