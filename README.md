...
| 장중 시장 압력 | 0.861, 광범위 하락 압력 |
| 현재 운영 판단 | 라이브 모델 유지, 서브 모델은 섀도 방식으로 성능 추적 |
| 최신 예측 상위권 | 금융, 자동차, 반도체/전자, 조선/방산, 유통/소비 |
| 실행 판단 | 전 섹터 약세로 최종 행동은 회피 우선, 반도체/전자와 조선/방산은 고위험 관찰 후보 |

## 핵심 성과 지표

| 지표 | 현재 값 | 해석 |
| --- | ---: | --- |
| 평가 예측 수 | 408건 | 2026-05-12부터 2026-07-07까지 누적 평가 |
| 방향 예측률 | 43.9% | 단순 상승/하락 방향은 아직 개선 필요 |
| Top3 겹침률 | 31.4% | 예측 상위 3개와 실제 상위권 겹침 정도 |
| RankIC 평균 | 0.027 | 섹터 순위 예측의 상관성은 낮은 양수 |
| NDCG@3 | 0.584 | 상위권 정렬 품질을 보는 보조 지표 |
| 수익률 구간 커버율 | 79.8% | 예상 수익률 구간 안에 실제 결과가 들어온 비율 |
...
2026-07-08 장마감 후 2026-07-09를 대상으로 한 예측입니다.
...
최근 순위 품질 게이트가 `severe`이고 장마감 기준 시장 압력이 강해, 관심 섹터 후보와 실제 진입 판단을 분리했습니다.
