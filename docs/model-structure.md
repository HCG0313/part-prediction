...
교수님 피드백 이후, 지금까지 만든 복합 예측 구조는 실험형 모델 기록으로 정리하고 새 메인 방향은 모멘텀 중심의 다음 거래일 상승/하락 예측 모델로 전환합니다.

기존 모델의 핵심 결과와 한계는 [기존 예측 모델 최종 정리](final-legacy-model-summary.md)에 따로 정리했습니다. 이 문서의 아래 내용은 기존 모델을 어떻게 발전시켰는지 보여주는 개발 이력으로 유지합니다.
